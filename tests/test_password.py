...
class TestRecoverPassword:
    ...
    def test_high_password(self, driver):
        ...
        login_page.send_email_to_input(login_page.locators.EMAIL_FIELD)
        login_page.click_recover_button()
        base_page.click_and_wait_element(login_page.locators.PASSWORD_FIELD)
        login_page.send_password_to_input(login_page.locators.PASSWORD_FIELD)
        base_page.click_and_wait_element(login_page.locators.HIDE_GLASS)
        active_field = WebDriverWait(login_page.driver, 5).until(
            EC.presence_of_element_located(login_page.locators.HIDE_GLASS))

        assert active_field.get_attribute, "Пароль не подсвечен"
